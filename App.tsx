...
function SafetyMeter({ advanced=false }) {
  ...
  function openDataCollector() {
    const params = new URLSearchParams({ wind: String(wind), humidity: String(humidity), dryness: String(dryness) });
  ...
  async function fetchFromCoords(lat, lon) {
    ...
    try {
      ...
      if (Number.isFinite(h)) setHumidity(Math.round(h));
      if (Number.isFinite(w)) setWind(Math.round(w));
      if (Number.isFinite(h)) setDryness(Math.max(0, Math.min(100, 100 - Math.round(h))));
      setLastFetch(`Updated from Open‑Meteo • ${new Date().toLocaleTimeString()}`);
    } catch (e) {
      setLastFetch(`Could not fetch live data (${e && e.message ? e.message : 'error'}).`);
    } finally {
      setBusy(false);
    }
  }

  async function fetchFromPlace(name) {
    setBusy(true);
    try {
      const geo = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(name)}&count=1&language=en&format=json`);
      if (!geo.ok) throw new Error(`Geo HTTP ${geo.status}`);
      const g = await geo.json();
      if (!g || !g.results || !g.results[0]) throw new Error('Place not found');
      const { latitude, longitude } = g.results[0];
      await fetchFromCoords(latitude, longitude);
    } catch (e) {
      setLastFetch(`Could not locate that place (${e && e.message ? e.message : 'error'}).`);
    } finally {
      setBusy(false);
    }
  }
  ...
  function useMyLocation() {
    if (!navigator.geolocation) { setLastFetch('Geolocation not available in this browser.'); return; }
    setBusy(true);
    navigator.geolocation.getCurrentPosition(
      (pos) => { const { latitude, longitude } = pos.coords; fetchFromCoords(latitude, longitude); },
      (err) => { setBusy(false); setLastFetch(`Location error: ${err && err.message ? err.message : err}`); },
  ...
  return (
    <div>
      <div style={{ display: "grid", gap: 8, gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))" }}>
        <label style={S.label}>Wind (mph): {wind}
          <input id="wind" type="range" min={0} max={30} value={wind} onChange={(e) => setWind(parseInt(e.target.value, 10))} style={{ width: "100%" }} />
        </label>
        <label style={S.label}>Humidity (%): {humidity}
          <input id="humidity" type="range" min={5} max={100} value={humidity} onChange={(e) => setHumidity(parseInt(e.target.value, 10))} style={{ width: "100%" }} />
        </label>
        <label style={S.label}>Dryness (0–100): {dryness}
          <input id="dryness" type="range" min={0} max={100} value={dryness} onChange={(e) => setDryness(parseInt(e.target.value, 10))} style={{ width: "100%" }} />
        </label>
      </div>

      <div style={{ ...S.row, marginTop: 8 }}>
        <input aria-label="Enter a city" placeholder="City or school name (e.g., Ann Arbor)" value={locInput} onChange={(e)=>setLocInput(e.target.value)} style={{ ...S.input, maxWidth: 260 }} />
        <button disabled={!locInput || busy} onClick={()=>fetchFromPlace(locInput)} style={{ ...S.btn, ...S.btnPrimary }}>{busy ? 'Fetching…' : 'Fetch Real‑Time'}</button>
        <button disabled={busy} onClick={useMyLocation} style={S.btn}>Use My Location</button>
        <button onClick={openDataCollector} style={{ ...S.btn }}>Open Data Collector</button>
      </div>
      ...
      <div role="status" aria-live="polite" style={{ marginTop: 10, display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <span style={S.small}>{lastFetch || (advanced ? "This classroom model uses 3 inputs (wind, humidity, dryness) similar to parts of a burn plan. Real burn decisions consider many more factors." : "This is a kid‑friendly model for learning. Only trained experts decide when and how to do a burn.")}</span>
      </div>
    </div>
  );
}
